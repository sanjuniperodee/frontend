...
export function useMarkers() {
  ...
  const {
    ...
    addMarker: addMarkerToStore,
    updateMarker: updateMarkerInStore,
    filters,
    getFilteredMarkers,
    fetchMarkers,
      connectToWebSocket
  ...
  const createMarker = useMutation({
    mutationFn: async (data: Omit<Marker, 'id' | 'createdAt' | 'approved' | 'rating'>) => {
  ...
  const rateMarker = useMutation({
    mutationFn: async ({ markerId, rating }: { markerId: string; rating: number }) => {
      const marker = markers.find(m => m.id === markerId);
      if (!marker) throw new Error('Метка не найдена');
      if(rating == 1){
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['markers'] });
      addPoints(
        variables.rating > 0 ? 2 : -1,
        variables.rating > 0 ? 'Положительная оценка метки' : 'Отрицательная оценка метки'
      );
    },
  });
