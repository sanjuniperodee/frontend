...
interface ReputationAction {
  type: 'marker_approved' | 'marker_rejected' | 'comment_liked' | 'report_accepted';
  userId: string;
  points: number;
}
...
export function useReputation() {
  ...
  const updateReputation = useMutation({
    mutationFn: async (action: ReputationAction) => {
    ...
    onSuccess: (data, variables) => {
      queryClient.setQueryData<User>(['user', variables.userId], (oldUser) => {
